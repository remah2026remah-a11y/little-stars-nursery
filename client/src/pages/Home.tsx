...
import { ArrowLeft, Baby, Camera, Check, ChevronDown, HeartHandshake, Menu, Phone, Play, ShieldCheck, Sparkles, Stethoscope, X } from "lucide-react";
...
export default function Home() {
  ...
  const reserve = () => { scrollTo("contact"); toast.success("رائع! سنكون بانتظار تواصلكم معنا 💛"); };
  return (
    <main dir="rtl" className="site-shell">
      ...
      <header className="nav-wrap">
        ...
        <nav className="nav-links">{navItems.map(([label, id]) => <button key={id} onClick={() => scrollTo(id)}>{label}</button>)}</nav>
        <div className="nav-actions"><a href="tel:+966500000000" className="phone"><Phone size={16} /> ٠٥٠ ٠٠٠ ٠٠٠٠</a><button className="primary-btn nav-cta" onClick={reserve}>احجزوا زيارة <ArrowLeft size={17} /></button></div>
        <button className="menu-btn" onClick={() => setMenuOpen(!menuOpen)} aria-label="فتح القائمة">{menuOpen ? <X /> : <Menu />}</button>
      </header>
      {menuOpen && <div className="mobile-menu">{navItems.map(([label, id]) => <button key={id} onClick={() => { scrollTo(id); setMenuOpen(false); }}>{label}</button>)}<button className="primary-btn" onClick={reserve}>احجزوا زيارة</button></div>}
      ...
      <section id="contact" className="contact-cta"><div><Sparkles size={20} /><h2>هل نبدأ <em>الحكاية؟</em></h2><p>زورونا لنتعرف على طفلكم ونريكم عالمه الجديد.</p></div><button className="primary-btn light-btn" onClick={() => toast.success("تم! سنتواصل معكم خلال دقائق لتأكيد الجولة.")}>احجزوا جولة مجانية <ArrowLeft size={18} /></button></section>
      <footer><div className="brand"><div className="brand-mark"><span>✦</span></div><div><strong>نجوم صغيرة</strong><small>حضانة ورعاية مبكرة</small></div></div><span>نزرع اليوم، ليزهروا غداً © 2024</span><div className="footer-links"><a href="tel:+966500000000">اتصلوا بنا</a><a href="#home">الخصوصية</a></div></footer>
